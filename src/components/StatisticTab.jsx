  ...
  return(
    <div className="rank-stack">
            ...
            <div key={s} className="site-col">
              ...
              <div className="tool-list">
                {data.map((item,i)=>{
                  ...
                  return(
                    <div key={i} className={`tool-item ${i===0&&!isLow?"active":""} ${isLow?"low-usage":""}`}>
                      ...
                      <div className="tool-name-text">{item.name}{isLow&&<span className="low-tag">{t("statistic.unused")}</span>}</div>
                ...
                <div className="tool-item" style={{borderTop:"2px solid var(--border-bright)",background:"rgba(0,212,255,0.04)",borderRadius:"0 0 8px 8px"}}>
                  ...
                  <div className="tool-name-text" style={{fontWeight:700,color:"var(--accent-cyan)"}}>{t("statistic.total")}</div>
      ...
      {(()=>{
        ...
        return(
        <div className="panel">
          <div className="panel-header"><div className="panel-title"><div className="panel-title-dot" style={{background:"var(--accent-amber)",boxShadow:"0 0 6px var(--accent-amber)"}}></div>{t("statistic.testerTitle")}</div></div>
          {top3.length>0&&(
            <div className="podium">
              {top3.map((item,i)=>{
                const barH=max>0?Math.max(20,Math.round(item.count/max*110)):20;
